...
func RankPrinter(r Ranker, i io.Writer) {
	// I had to get help for this one from the exercise solution

	// get the results of the function call
	teams := r.Ranking()

	// for each team, print it out to designated target
	for _, v := range teams {
		io.WriteString(i, v)
		i.Write([]byte("\n"))
	}
...
func main() {
	...
	// https://www.geeksforgeeks.org/io-writestring-function-in-golang-with-examples/
	//	-- first supported variable here must be of the Ranker interface type
	//	-- second must be of the io.Writer type, which os.Stdout is
	RankPrinter(the_league, os.Stdout)
